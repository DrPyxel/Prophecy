...
{
    public class MustardGas : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            if (projectile.ai[0] > 12f)  //this defines where the flames starts
            {
                ...
                {
                    Dust.NewDust(new Vector2(projectile.position.X, projectile.position.Y), projectile.width, projectile.height, DustID.Grass, 0f, 0f, 220, default(Color), 1f);
                    Dust.NewDust(new Vector2(projectile.position.X, projectile.position.Y), projectile.width, projectile.height, 19, 0f, 0f, 220, default(Color), 1f);   //this defines the flames dust and color, change DustID to wat dust you want from Terraria, or add mod.DustType("CustomDustName") for your custom dust
                }
            }
            else
            {
        ...
        public override void Kill(int timeLeft)
        {
            ...
            for (int d = 0; d < 30; d++)
            {
                ...
                Dust.NewDust(new Vector2(projectile.position.X, projectile.position.Y), projectile.width, projectile.height, 44, 0f, 0f, 125, default(Color), 0.5f);
            }
        }
    }
}
